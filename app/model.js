...
module.exports = function (str) {
    ...
    var isFMAAT = str.match(/[F]?(MAAT)/g) && str.match(/[F]?(MAAT)/g).indexOf("FMAAT") != -1 ? true : false;
    var isSAT = str.indexOf("SAT") != -1 ? true : false
    var isMBA = str.indexOf("MBA") != -1 ? true : false
    var isMAAT = str.match(/[F]?(MAAT)/g) && str.match(/[F]?(MAAT)/g).indexOf("MAAT") != -1 ? true : false;
    ...
    var isOfficeTel = office_str.search("Tel No:") != -1 ? true : false
    var isOfficeFax = office_str.search("Fax:") != -1 ? true : false
    var isOfficeEmail = office_str.search("E-Mail:") != -1 ? true : false
    ...
    if (isOfficeContent) {
        ...
        // OFFICE tel
        if (isOfficeTel) {
            var re =  /(.*Tel No:)(.*?)((\s?Fax:.*)|(\s?E-Mail:.*))/;
            console.log("CORRECT? - " + re.exec(office_str)[2])
    ...
    var isResidenceTel = residence_str.search("Tel No:") != -1 ? true : false
    var isResidenceFax = residence_str.search("Fax:") != -1 ? true : false
    var isResidenceEmail = residence_str.search("E-Mail:") != -1 ? true : false
    ...
    // Residence addr
    var re = /.*RESIDENCE(.*?)((\s?Tel No:.*)|(\s?Fax:.*)|(\s?E-Mail:.*))/;
    var newtext = str.slice(0).replace(re, "$1");
    ...
    if (isResidenceContent) {
        ...
        if (isResidenceTel) {
            ...
            var newtext = newtext.split("E-Mail:")[0]
